...
In-class work is done in groups during the session and posted once, by one person, to that session's forum. I expect every member of the group to contribute. Everyone named in the post gets the same score. If your name is not in the post, you get no credit for that session, so check the names before it goes up. Times given for in-class work are approximate; I set the clock in the session, and when your group has posted, you are done for the day. In-class work cannot be made up, because it is group work done live; that is the attendance policy.
...
If you used AI on an assignment, add one line at the end saying how. Saying so never costs points; the work is graded the same either way. Not saying so, when it is obvious or an AI check flags it, is an honesty problem. See Academic honesty below.
