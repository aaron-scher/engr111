...
In-class work is done in groups during the session and posted once, by one person, to that session's forum. I expect every member of the group to contribute. Everyone named in the post gets the same score. If your name is not in the post, you get no credit for that session, so check the names before it goes up. Times given for in-class work are approximate; I set the clock in the session. When the rooms close we come back together and go through the posts, so be ready to speak for your group. In-class work cannot be made up, because it is group work done live; that is the attendance policy.
